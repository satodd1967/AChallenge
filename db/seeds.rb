...
# rank =  LogScore.group(:challenge_goal).sum(:total_points)

# c = Challenge.first
# rank = c.log_scores.group(:challenge_goal_id).sum(:total_points) as "tot"
# rank = ChallengeGoal.joins(:log_scores).group('challenge_goals.id').order('SUM(log_scores.total_points)')

# rank = ChallengeGoal.joins(:log_scores).group(:challenge_goal_id).sum(:total_points)

# rank = ChallengeGoal.joins(:log_scores).group(:challenge_goal_id).order('sum_total_points DESC').sum(:total_points)

# rank = ChallengeGoal.where(challenge_id: c.id).joins(:log_scores).group(:challenge_goal_id).order('sum_total_points DESC').sum(:total_points)


c = Challenge.first
rank = ChallengeGoal.where(challenge_id: c.id).joins(:log_scores).group(:challenge_goal_id).order('sum_total_points DESC').sum(:total_points)
rank.each_with_index do |place, i|
    puts "#{ChallengeGoal.find_by(id: place[0]).user.username} #{i + 1} - #{place[1]}"
end   
